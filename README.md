...
VibeGames is a browser-first reinforcement learning playground where you can watch, tune, and train a DQN agent in real-time. No server-side ML required — everything runs on your CPU via JavaScript.
...
│   ├── src/
│   │   ├── components/         # GameCanvas, ControlPanel, Leaderboard, etc.
│   │   ├── games/
│   │   │   └── flappy/         # Flappy Bird engine, renderer, physics
│   │   ├── rl/                 # DQN agent, neural net, replay buffer
│   │   ├── services/           # API client for leaderboard
│   │   ├── views/              # Landing page, training view
│   │   └── router/             # Vue Router (multi-game routing)
│   ├── public/
│   │   └── assets/             # Sprites and audio
│   └── server/                 # Leaderboard API (Node/Caddy dockerized)
│
├── FlapPyBird/                 # 📚 Original Python implementation (reference)
│   ├── rl/                     # Python DQN implementation
│   ├── src/                    # Python game engine
│   └── checkpoints/            # Trained model weights
│
├── deploy.sh                   # Deployment script (VibeGames static + API)
├── docker-compose.yml          # Docker configuration (generated by deploy)
└── Caddyfile                   # Caddy web server config (generated by deploy)
```
