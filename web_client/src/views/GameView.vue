    ...
    <header class="app-header">
      <h1 class="app-title text-display">
        <router-link to="/" class="home-link">
          <span class="text-primary glow">{{ gameTitle }}</span>
    ...
    <main class="app-main">
      ...
      <aside class="sidebar">
        ...
        <div class="sidebar-panels">
          ...
          <LeaderboardPanel v-if="mode === 'idle'" :gameId="gameId" />
...
export default defineComponent({
  ...
  methods: {
    ...
    handleAutoEvalResult(result: AutoEvalResult) {
      console.log('[GameView] Auto-eval result:', result)
    ...
    handleArchitectureLoaded(hiddenLayers: number[]) {
      ...
      this.hasModel = true
      this.isPaused = true
      console.log('[GameView] Checkpoint loaded, model ready for eval')
    },
    handleBackendReady(backend: BackendType) {
      this.backend = backend
      console.log('[GameView] Backend ready:', backend)
    },
      ...
      reader.onload = async () => {
        ...
        if (text && gameCanvas && (gameCanvas as any).loadCheckpointFromJSON) {
          try {
            ...
            console.error('[GameView] Failed to load checkpoint from JSON:', error)
          }
        }
      }
      ...
      if (newMode === 'eval') {
        ...
        if (gameCanvas) {
          ...
          try {
            ...
            console.error('[GameView] Failed to start eval:', error)
          }
        }
        ...
        if (gameCanvas) {
          try {
            ...
            console.error('[GameView] Failed to start training:', error)
          }
        }
      ...
      if (this.mode === 'manual') {
        ...
        if (gameCanvas) {
          ...
          try {
            ...
            console.error('[GameView] Failed to restart training:', error)
          }
        }
      }
    },
    ...
    async backToTraining() {
      ...
      if (gameCanvas) {
        ...
        try {
          ...
          console.error('[GameView] Failed to start training (backToTraining):', error)
        }
      }
    },
    async restartEval() {
      ...
      if (gameCanvas) {
        try {
          ...
          console.error('[GameView] Failed to restart eval:', error)
        }
      }
    },
    ...
    handleGameChange() {
      // Reset UI and training state when switching games
      this.resetTraining()
      this.mode = 'idle'
      this.showGameOver = false
      this.manualEvalActive = false
      this.autoEvalActive = false
      // Refresh leaderboard threshold for the new game
      this.refreshLeaderboardThreshold()
      // Ensure canvas stops any running game
      const gameCanvas = this.$refs.gameCanvas as InstanceType<typeof GameCanvas>
      if (gameCanvas) {
        gameCanvas.stopGame()
      }
